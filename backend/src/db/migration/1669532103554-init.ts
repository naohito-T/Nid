...
export class init1669532103554 implements MigrationInterface {
    name = 'init1669532103554'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE "terms" ("id" SERIAL NOT NULL, "created_at" TIMESTAMP(3) NOT NULL DEFAULT now(), "updated_at" TIMESTAMP(3) NOT NULL DEFAULT now(), "term_version" TIMESTAMP(3) DEFAULT now(), "userId" uuid, CONSTRAINT "REL_e3371ddb00d1d22b2ee73a576f" UNIQUE ("userId"), CONSTRAINT "PK_33b6fe77d6ace7ff43cc8a65958" PRIMARY KEY ("id")); COMMENT ON COLUMN "terms"."term_version" IS '利用規約の最新versionを保持する（時間orTimeStamp）'`);
        await queryRunner.query(`CREATE TYPE "public"."users_sex_enum" AS ENUM('0', '1', '2')`);
        await queryRunner.query(`CREATE TABLE "users" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "created_at" TIMESTAMP(3) NOT NULL DEFAULT now(), "updated_at" TIMESTAMP(3) NOT NULL DEFAULT now(), "first_name" character varying NOT NULL, "last_name" character varying NOT NULL, "birth_date" character varying, "sex" "public"."users_sex_enum" NOT NULL, "nick_name" character varying NOT NULL, "telephone_number" character varying NOT NULL, "email" character varying NOT NULL, "thumbnail_url" character varying, "user_addresses" character varying array, "user_authorization" character varying array NOT NULL, "has_term_version" TIMESTAMP array, CONSTRAINT "UQ_97672ac88f789774dd47f7c8be3" UNIQUE ("email"), CONSTRAINT "PK_a3ffb1c0c8416b9fc6f907b7433" PRIMARY KEY ("id")); COMMENT ON COLUMN "users"."first_name" IS '苗字'; COMMENT ON COLUMN "users"."last_name" IS '名前'; COMMENT ON COLUMN "users"."birth_date" IS '生年月日'; COMMENT ON COLUMN "users"."sex" IS '性別 0: 男性 1: 女性 2: 回答なし'; COMMENT ON COLUMN "users"."nick_name" IS 'ニックネーム'; COMMENT ON COLUMN "users"."telephone_number" IS '電話番号'; COMMENT ON COLUMN "users"."email" IS 'メールアドレス'; COMMENT ON COLUMN "users"."thumbnail_url" IS 'サムネイル'; COMMENT ON COLUMN "users"."user_addresses" IS 'ユーザが持っている住所（複数持てる）'; COMMENT ON COLUMN "users"."user_authorization" IS '認証したフロー（複数ある想定）'; COMMENT ON COLUMN "users"."has_term_version" IS '同意している利用規約のversion（空白の場合は同意していないと同義）'`);
